...
/*   Updated: 2024/09/19 19:38:53 by gecarval         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector vectordiv(t_vector v, float_t scalar) {
	if (scalar != 0) return (create_vector(v.x / scalar, v.y / scalar));
...
t_vector vector_setmagdiv(t_vector v, float_t magnitude) {
	...
	if (magnitude != 0) return (vectordiv(v, mag / magnitude));
	return (v);
}
...
t_vector vector_setmagmult(t_vector v, float_t magnitude) {
	...
	if (mag != 0) return (vectormult(v, magnitude / mag));
	return (v);
}

t_vector mirror_forces(t_vector v, t_data *data) {
	if (v.x > 0.5 * data->winx) v.x -= data->winx;
	if (v.x < -0.5 * data->winx) v.x += data->winx;
	if (v.y > 0.5 * data->winy) v.y -= data->winy;
	if (v.y < -0.5 * data->winy) v.y += data->winy;
	return (v);
}
...
t_vector limit_vector(t_vector v, t_data *data) {
	if (v.x > data->winx) v.x = 0;
	if (v.y > data->winy) v.y = 0;
	if (v.x < 0) v.x = data->winx;
	if (v.y < 0) v.y = data->winy;
	return (v);
}
