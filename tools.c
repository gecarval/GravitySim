...
t_particle *search_heaviest_particle(t_data *data) {
	...
	heaviest = tmp;
	while (tmp != NULL) {
		if (tmp->mass > heaviest->mass) heaviest = tmp;
		tmp = tmp->next;
	}
	return (heaviest);
}
...
void put_all_particles_in_orbit(t_data *data) {
	...
	while (tmp != NULL) {
		if (tmp != heaviest) {
			...
			tmp->vel =
				vectoradd(tmp->vel, create_vector(cos(angle) * strength,
												  sin(angle) * strength));
		}
		tmp = tmp->next;
	}
...
void put_particle(int x, int y, t_data *data) {
	...
	if (!tmp) display_error(data, "part malloc error\n");
	tmp->next = NULL;
	pthread_mutex_init(&tmp->mutex, NULL);
	tmp->pos = create_vector(x, y);
	tmp->prev_pos = tmp->pos;
	tmp->vel =
		create_vector(data->mposx - data->pmposx, data->mposy - data->pmposy);
...
int mlx_cooked(int key, t_data *data) {
	...
	if (key == '1') data->anigsim *= -1;
	if (key == ']') data->timing += 10000000;
	if (key == '[')
		if (data->timing > 9999999) data->timing -= 10000000;
	if (key == 't') reset_pos(data);
	if (key == 'y') data->show_tree *= -1;
	if (key == 'u') data->show_collision_tree *= -1;
	if (key == ' ') {
...
void pixel_to_img(int x, int y, t_data *data, int color) {
	...
	if ((x < 0 || x > data->winx) || (y < 0 || y > data->winy)) return;
...
void render_background(t_data *data, int color) {
	...
	while (++x < data->winx) {
		y = -1;
		while (++y < data->winy) pixel_to_img(x, y, data, color);
	}
}
