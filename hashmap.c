...
t_hashmap **create_hashmap(t_data *data) {
	...
	if (hashmap == NULL) display_error(data, "hashmap malloc error\n");
	i = -1;
	while (++i < HASHMAP_DIV) {
		hashmap[i] = (t_hashmap *)malloc(sizeof(t_hashmap) * HASHMAP_DIV);
		if (hashmap[i] == NULL) display_error(data, "hashmap malloc error\n");
		j = -1;
		while (++j < HASHMAP_DIV) {
			hashmap[i][j].hashquad =
				create_rectangle((((float_t)data->winx / HASHMAP_DIV) * j) +
									 (((float_t)data->winx / HASHMAP_DIV) / 2),
								 (((float_t)data->winy / HASHMAP_DIV) * i) +
									 (((float_t)data->winy / HASHMAP_DIV) / 2),
								 (float_t)data->winx / HASHMAP_DIV,
								 (float_t)data->winy / HASHMAP_DIV);
...
void insert_points_hashmap(t_hashmap **hashmap, t_data *data) {
	...
	while (++i < data->num_of_particles) {
		...
		if (key == NULL) display_error(data, "hashkey malloc error\n");
		tmp2 = (t_point *)malloc(sizeof(t_point));
		if (tmp2 == NULL) display_error(data, "point malloc error\n");
		tmp2->x = tmp->pos.x;
		tmp2->y = tmp->pos.y;
		tmp2->part = tmp;
		key->point = tmp2;
		...
		if (posy >= HASHMAP_DIV) posy = HASHMAP_DIV - 1;
		if (posy < 0) posy = 0;
		if (posx >= HASHMAP_DIV) posx = HASHMAP_DIV - 1;
		if (posx < 0) posx = 0;
...
void free_hashkey(t_hashkey *hashkey) {
	...
	while (hashkey != NULL) {
		...
		if (tmp->point != NULL) free(tmp->point);
