...
t_point create_point(float_t x, float_t y, t_particle *part) {
	return ((t_point){x, y, part});
}
...
void highlight_point(t_point pt, int size, t_data *data) {
	...
	i = abs(size);
	while (--i >= 0) circlebres((int)pt.x, (int)pt.y, i, data, 0x00FF00);
}

// RECTANGLE HELPERS FUNCTIONS
t_rectangle create_rectangle(float_t x, float_t y, float_t w, float_t h) {
	return (
		(t_rectangle){x, y, w, h, x - w / 2, x + w / 2, y - h / 2, y + h / 2});
}
...
int cirintersects(t_circle *c, t_rectangle *range) {
	...
	if (xdist > c->r + w || ydist > c->r + h) return (0);
...
t_quadtree *create_quadtree(t_rectangle boundary, unsigned int capacity,
	...
	if (!qt->points) free(qt);
	if (!qt->points) return (NULL);
	i = 0;
	tmp = qt->points;
	while (i < capacity) {
		tmp++;
...
void subdivide_tree(t_quadtree *qt) {
	...
	while (++i < qt->point_count) {
		p = qt->points[i];
		inserted =
			insert_point(qt->northeast, p) || insert_point(qt->northwest, p) ||
...
bool insert_point(t_quadtree *qt, t_point p) {
	if (!qt) return (0);
	if (!quadcontains(&qt->boundary, &p)) return (0);
	if (qt->point_count < qt->capacity && qt->depth < MAX_DEPTH &&
		!qt->divided) {
		qt->points[qt->point_count++] = p;
		return (1);
	}
	if (!qt->divided && qt->depth < MAX_DEPTH) subdivide_tree(qt);
...
void query_quadtree(t_quadtree *qt, t_rectangle *range, t_point *found[],
	...
	if (!quadintersects(&qt->boundary, range)) return;
	i = -1;
	while (++i < qt->point_count)
...
void query_quadtree_circle(t_quadtree *qt, t_circle *range, t_point *found[],
	...
	if (!qt->points) return;
	if (!cirintersects(range, &qt->boundary)) return;
	i = 0;
	while (i < qt->point_count) {
